...
local GetNumAddOns        = _G.GetNumAddOns        or function() return 0 end
local IsAddOnLoaded       = _G.IsAddOnLoaded       or function() return false end
local GetAddOnInfo        = _G.GetAddOnInfo
local GetAddOnCPUUsage    = _G.GetAddOnCPUUsage    or function() return 0 end
local GetAddOnMemoryUsage = _G.GetAddOnMemoryUsage or function() return 0 end
...
SlashCmdList["FPSGRAPH"] = function(msg)
    ...
    elseif msg:match("^sample%s+(%d+)") then
        local s = tonumber(msg:match("%d+"))
        FPSMonitorDB.sampleInterval = s
        sampleInterval = s
        print("FPSMonitor: sample interval set to "..s.."s")
    elseif msg == "export" then
        FPSMonitorExport = CopyTable(graphHistory)
        print("FPSMonitor: History exported to SavedVariables.FPSMonitorExport")
    elseif msg == "reset" then
        for k,_ in pairs(graphHistory) do graphHistory[k] = {} end
        graphCount, graphStart = 0, 1
        print("FPSMonitor: graph history reset.")
    elseif msg == "help" then
        print("/fpsgraph toggle     - show/hide graph")
        print("/fpsgraph window <s> - set time window")
        print("/fpsgraph sample <s> - set sample interval")
        print("/fpsgraph export     - export history to SavedVariables")
        print("/fpsgraph reset      - clear graph data")
    end
end
...
function CreateGraphFrame()
    ...
    if FPSMonitorDB.graph.position then
        graphFrame:ClearAllPoints()
        graphFrame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT",
                           FPSMonitorDB.graph.position.x,
                           FPSMonitorDB.graph.position.y + FPSMonitorDB.graph.position.h)
        graphFrame:SetSize(FPSMonitorDB.graph.position.w, FPSMonitorDB.graph.position.h)
    end
    ...
    graphFrame:SetScript("OnDragStop", function(self)
        ...
        local gx, gy = self:GetLeft(), self:GetTop()
        FPSMonitorDB.graph.position = { x = gx, y = gy, w = self:GetWidth(), h = self:GetHeight() }
    end)
    -- Resizer handle
    ...
    sizer:SetScript("OnMouseUp", function()
        ...
        local gx, gy = graphFrame:GetLeft(), graphFrame:GetTop()
        FPSMonitorDB.graph.position = { x = gx, y = gy, w = graphFrame:GetWidth(), h = graphFrame:GetHeight() }
    end)
    ...
    graphFrame:SetScript("OnEnter", function(self)
        ...
        if GetNumAddOns and GetAddOnMemoryUsage and GetAddOnCPUUsage then
            UpdateAddOnMemoryUsage()
            UpdateAddOnCPUUsage()
            local memList, cpuList = {}, {}
            for i=1,GetNumAddOns() do
                if IsAddOnLoaded(i) then
                    local name = GetAddOnInfo(i)
                    table.insert(memList,{name=name,mem=GetAddOnMemoryUsage(i)})
                    table.insert(cpuList,{name=name,cpu=GetAddOnCPUUsage(i)})
                end
            end
            table.sort(memList,function(a,b) return a.mem>b.mem end)
            table.sort(cpuList,function(a,b) return a.cpu>b.cpu end)
            GameTooltip:AddLine("Top Memory:")
            for i=1,math.min(5,#memList) do
                GameTooltip:AddDoubleLine(memList[i].name,string.format("%.1f MB",memList[i].mem/1024))
            end
            GameTooltip:AddLine("Top CPU:")
            for i=1,math.min(5,#cpuList) do
                GameTooltip:AddDoubleLine(cpuList[i].name,string.format("%.2f ms",cpuList[i].cpu))
            end
        end
        GameTooltip:Show()
    end)
    ...
    graphFrame:SetScript("OnMouseMove", function(self, x, y)
        ...
        GameTooltip:AddLine(string.format("Mem: %.2f MB", graphHistory.memory[idx]))
        GameTooltip:AddLine(string.format("Lat: %.0f ms", graphHistory.latency[idx]))
        GameTooltip:Show()
    end)
    ...
    graphFrame:SetScript("OnMouseWheel", function(_, delta)
        ...
        FPSMonitorDB.graph.timeWindow = math.max(10, math.min(300, w))
        UpdateGraphConfig()
    end)
...
local function ValidateConfig()
    ...
    if type(FPSMonitorDB.graph) ~= "table" then
        ...
        local g = FPSMonitorDB.graph
        if type(g.enabled) ~= "boolean" then g.enabled = defaultConfig.graph.enabled end
        if type(g.w) ~= "number" then g.w = defaultConfig.graph.w end
        if type(g.h) ~= "number" then g.h = defaultConfig.graph.h end
        if type(g.timeWindow) ~= "number" or g.timeWindow <= 0 then g.timeWindow = defaultConfig.graph.timeWindow end

        if type(g.pos) ~= "table" then
            g.pos = CopyTable(defaultConfig.graph.pos)
        else
            local p = g.pos
            if type(p.point) ~= "string" then p.point = defaultConfig.graph.pos.point end
            if type(p.relativePoint) ~= "string" then p.relativePoint = defaultConfig.graph.pos.relativePoint end
            if type(p.x) ~= "number" then p.x = defaultConfig.graph.pos.x end
            if type(p.y) ~= "number" then p.y = defaultConfig.graph.pos.y end
        end

        if type(g.showFPS) ~= "boolean" then g.showFPS = defaultConfig.graph.showFPS end
        if type(g.showFrameTime) ~= "boolean" then g.showFrameTime = defaultConfig.graph.showFrameTime end
        if type(g.showJitter) ~= "boolean" then g.showJitter = defaultConfig.graph.showJitter end
        if type(g.showMemory) ~= "boolean" then g.showMemory = defaultConfig.graph.showMemory end
        if type(g.showLatency) ~= "boolean" then g.showLatency = defaultConfig.graph.showLatency end

        if type(g.yMin) ~= "number" and g.yMin ~= nil then g.yMin = defaultConfig.graph.yMin end
        if type(g.yMax) ~= "number" and g.yMax ~= nil then g.yMax = defaultConfig.graph.yMax end
        for _, k in ipairs({"FrameTime", "Memory", "Latency", "Jitter"}) do
            local minKey = "yMin" .. k
            local maxKey = "yMax" .. k
            if type(g[minKey]) ~= "number" and g[minKey] ~= nil then
                g[minKey] = defaultConfig.graph[minKey]
            end
            if type(g[maxKey]) ~= "number" and g[maxKey] ~= nil then
                g[maxKey] = defaultConfig.graph[maxKey]
            end
        end

        if type(g.smoothing) ~= "number" then g.smoothing = defaultConfig.graph.smoothing end
        if type(g.autoScale) ~= "boolean" then g.autoScale = defaultConfig.graph.autoScale end
        if type(g.timeTicks) ~= "number" or g.timeTicks <= 0 then g.timeTicks = defaultConfig.graph.timeTicks end
        if type(g.showPercentiles) ~= "boolean" then g.showPercentiles = defaultConfig.graph.showPercentiles end
        if type(g.alertThreshold) ~= "number" then g.alertThreshold = defaultConfig.graph.alertThreshold end
        if type(g.graphSampleResolution) ~= "number" or g.graphSampleResolution <= 0 then
            g.graphSampleResolution = defaultConfig.graph.graphSampleResolution
        end
        if type(g.hideInCombat) ~= "boolean" then g.hideInCombat = defaultConfig.graph.hideInCombat end
    end
...
local function AddSample(dt)
    if graphPaused then return end
...
local function CalculateStats(currentFPS, currentDT)
    ...
    for i = historyStart, historyStart + historyCount - 1 do
        ...
        InsertSorted(low5List, sample.fps, low5Size)
        fpsList[#fpsList+1] = sample.fps
    end
    table.sort(fpsList)
    ...
    local low5 = low5List[low5Size] or low5List[#low5List]
    local median = fpsList[math_floor(#fpsList * 0.5 + 0.5)] or currentFPS
...
function UpdateGraph()
    if graphPaused or not graphFrame or not graphFrame:IsShown() or graphCount < 2 then return end
    ...
    local startTime = graphHistory.times[graphStart]
    local graphW = graphFrame:GetWidth() - 4

    local metrics = {
        { key = "fps",        data = graphHistory.fps,         lines = graphLines.fps,         color = FPSMonitorDB.graph.colorFPS or {0,1,0},   thick = FPSMonitorDB.graph.thickFPS or 1.5, flag = FPSMonitorDB.graph.showFPS },
    ...
    local minFPS, maxFPS
    if FPSMonitorDB.graph.autoScale then
        minFPS, maxFPS = math.huge, -math.huge
        for _, m in ipairs(metrics) do
            for i = 1, sampleCount do
                local v = m.plot[i]
                if v then
        ...
        local pad = (maxFPS - minFPS) * 0.1
        if pad == 0 then pad = 1 end
        minFPS = minFPS - pad
        maxFPS = maxFPS + pad
    else
        minFPS = FPSMonitorDB.graph.yMin or 0
        maxFPS = FPSMonitorDB.graph.yMax or 1
    ...
    local fpsRange = maxFPS - minFPS
        ...
        if FPSMonitorDB.graph.showPercentiles then
            ...
            local yMed = ((stats.median - minFPS) / fpsRange) * height + 2
            ...
            graphGrid.p01:Show()
            graphGrid.pMed:SetStartPoint("BOTTOMLEFT",2,yMed)
            graphGrid.pMed:SetEndPoint("BOTTOMRIGHT",-2,yMed)
            graphGrid.pMed:Show()
        else
    ...
    for _, m in ipairs(metrics) do
        ...
        for i = 1, sampleCount do
            ...
            local val = m.plot[i]
            if val and idx then
                local t1 = graphHistory.times[idx] - startTime
                local x = (t1 / graphTimeWindow) * graphW + 2
    ...
    local ticks = FPSMonitorDB.graph.timeTicks or 4
    for i = 0, ticks do
        local x = (i / ticks) * width + 2
        ...
        if label then
            local interval = graphTimeWindow / ticks
            local secs = math.floor(graphTimeWindow - i * interval)
            label:SetPoint("BOTTOMLEFT", graphFrame, "BOTTOMLEFT", x-6, 4)
            label:SetText(string.format("-%ds", secs))
            label:Show()
        end
    end
    for i = ticks + 2, #graphGrid.v do
        if graphGrid.v[i] then graphGrid.v[i]:Hide() end
        if graphLabels.v[i] then graphLabels.v[i]:Hide() end
    end
...
local function CreateOptionsPanel()
    ...
    local combatChk = CreateFrame("CheckButton", nil, optionsPanel.general, "UICheckButtonTemplate")
    combatChk:SetPoint("TOPLEFT", pctChk, "BOTTOMLEFT", 0, -4)
    if combatChk.Text then combatChk.Text:SetText("Hide graph in combat") end
    combatChk:SetChecked(FPSMonitorDB.graph.hideInCombat)
    combatChk:SetScript("OnClick", function(self)
        FPSMonitorDB.graph.hideInCombat = self:GetChecked()
    end)

    local alertSlider = CreateFrame("Slider", nil, optionsPanel.general, "OptionsSliderTemplate")
    alertSlider:SetPoint("TOPLEFT", combatChk, "BOTTOMLEFT", 0, -30)
    ...
    -- Color pickers and thickness sliders
    local metricOptions = {
        {label="FPS", color="colorFPS", thick="thickFPS"},
        {label="Frame", color="colorFrameTime", thick="thickFrameTime"},
        {label="Jitter", color="colorJitter", thick="thickJitter"},
        {label="Memory", color="colorMemory", thick="thickMemory"},
        {label="Total", color="colorTotalMemory", thick="thickTotalMemory"},
        {label="AllMem", color="colorTotalMem", thick="thickTotalMem"},
        {label="LuaMem", color="colorLuaMem", thick="thickLuaMem"},
    ...
    local function CreateColorOption(info)
        local btn = CreateFrame("Button", nil, optionsPanel.general)
        btn:SetSize(16,16)
        btn:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -8)
        btn.tex = btn:CreateTexture(nil,"BACKGROUND")
        btn.tex:SetAllPoints()
        btn.tex:SetColorTexture(unpack(FPSMonitorDB.graph[info.color]))
        btn:SetScript("OnClick", function()
            local r,g,b = unpack(FPSMonitorDB.graph[info.color])
            ColorPickerFrame.func = function()
                local nr,ng,nb = ColorPickerFrame:GetColorRGB()
                FPSMonitorDB.graph[info.color] = {nr,ng,nb}
                btn.tex:SetColorTexture(nr,ng,nb)
                UpdateGraph()
            end
            ColorPickerFrame:SetColorRGB(r,g,b)
            ColorPickerFrame:Hide(); ColorPickerFrame:Show()
        end)
        local lbl = optionsPanel.general:CreateFontString(nil,"ARTWORK","GameFontHighlightSmall")
        lbl:SetPoint("LEFT", btn, "RIGHT", 4, 0)
        lbl:SetText(info.label .. " color")

        local slider = CreateFrame("Slider", nil, optionsPanel.general, "OptionsSliderTemplate")
        slider:SetPoint("LEFT", lbl, "RIGHT", 20, 0)
        slider:SetMinMaxValues(0.5,4)
        slider:SetValueStep(0.5)
        slider:SetObeyStepOnDrag(true)
        slider:SetWidth(120)
        slider:SetValue(FPSMonitorDB.graph[info.thick])
        ...
        slider.text:SetText(string.format("%.1f", FPSMonitorDB.graph[info.thick]))
        slider:SetScript("OnValueChanged", function(self,v)
            v = math.floor(v*10+0.5)/10
            self.text:SetText(string.format("%.1f", v))
            FPSMonitorDB.graph[info.thick] = v
            UpdateGraph()
        end)
        anchor = slider
    end
    for _,info in ipairs(metricOptions) do
        CreateColorOption(info)
    end
...
local function OnEvent(_, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        do
            ...
            local bwIn, bwOut, homeLat, worldLat = GetNetStats()
        ...
        memInterval = memoryUpdateInterval
    ...
    elseif event == "PLAYER_REGEN_DISABLED" then
        if FPSMonitorDB.graph.hideInCombat and graphFrame then
            graphFrame:Hide()
        end
    elseif event == "PLAYER_REGEN_ENABLED" then
        if FPSMonitorDB.graph.hideInCombat and FPSMonitorDB.graph.enabled and graphFrame then
            graphFrame:Show()
        end
